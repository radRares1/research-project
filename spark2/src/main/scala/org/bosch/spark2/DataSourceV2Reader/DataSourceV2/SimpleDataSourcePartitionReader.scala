...
//  val recordExprEncoder: ExpressionEncoder[Record] = recordEncoder.asInstanceOf[ExpressionEncoder[Record]]
  ...
  def recordToSeq(record:Record):Seq[Any] = {
    Seq(UTF8String.fromString(record.filename),Array(UTF8String.fromString(record.parameter.name),
      UTF8String.fromString(record.parameter.unit)),record.timeArray.clone(),record.valueArray.clone())
  }
